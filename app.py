...
class calculator:
    def streamlit_config(self):
        ...
        st.markdown('<h1 style="text-align: center;">Virtual Calculator</h1><p style="text-align:center;color:#777;">✍️ Draw. 👁️ Watch. 🤖 Analyze.</p>', unsafe_allow_html=True)
        add_vertical_space(1)

        # Show gesture controls at bottom
        st.markdown("""
        <hr>
        <h5>Gesture‑Based Controls:</h5>
        <ul>
          <li>✍️ <b>Draw Math Problems:</b> Lift Thumb + Index</li>
          <li>🖱️ <b>Move Around:</b> Lift Thumb + Index + Middle</li>
          <li>🧽 <b>Erase Content:</b> Lift Thumb + Middle</li>
          <li>🗑️ <b>Reset Canvas:</b> Lift Thumb + Pinky</li>
          <li>📤 <b>Send to AI Model:</b> Lift Index + Middle</li>
        </ul>
        <hr><br>
...
if __name__ == "__main__":
    ...
    while True:
        app.process_frame()
        app.process_hands()
        app.identify_fingers()
        app.handle_drawing_mode()
        app.blend_canvas_with_feed()

        # Show the result in OpenCV window (if needed)
        cv2.imshow("Virtual Calculator", app.img)
